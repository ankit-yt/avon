...
function setTranslateCookie(langCode) {
  ...
  if (domain.includes(".")) {
    ...
    document.cookie = `googtrans=${cookieValue}; path=/; domain=${dotDomain};`;
  }
}
...
export default function LanguageSwitcher() {
  ...
  return (
    <div ref={dropdownRef} className="relative font-sans text-sm" onKeyDown={handleKeyDown}>
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      >
        <span className="text-xs font-bold uppercase tracking-widest">{selected.name}</span>
        <svg
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-100 rounded-xl shadow-xl z-[60] overflow-hidden">
          <div className="p-2 bg-slate-50 border-b border-slate-100">
            <input
              ...
              className="w-full bg-white text-slate-900 text-xs px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-1 focus:ring-orange-500"
            />
          </div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((lang, idx) => (
                <div
                  ...
                  className={`px-4 py-2.5 cursor-pointer text-xs font-medium flex items-center justify-between transition-colors
                    ${selected.code === lang.code ? "text-orange-600 bg-orange-50" : "text-slate-600"}
                    ${focusedIdx === idx ? "bg-slate-100 text-slate-900" : "hover:bg-slate-50 hover:text-slate-900"}
                  `}
                >
                  {lang.name}
